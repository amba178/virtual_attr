class Product < ActiveRecord::Base
  ...
  def price_in_dollars
  		self.price_in_cents.to_d/100 if price_in_cents 
  end
  ...
  def check_released_at_text
    if @released_at_text.present? && Time.zone.parse(@released_at_text).nil? || @released_at_text.empty?
      errors.add :released_at_text, "cannot be parsed" if @released_at_text.present? 
      errors.add :released_at_text, "cannot be blank"  if @released_at_text.empty?
    end
